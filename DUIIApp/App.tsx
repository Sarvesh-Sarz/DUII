...
export default function App() {
  ...
  const simulateFetch = async () => {
    ...
    try {
      ...
      const data = await NativeModules.UsageStatsModule.getUsageData();

      if (data.permissionNeeded) {
        // Ask user to grant permission then open settings
        Alert.alert(
          'Permission Required',
          'DUII needs Usage Access permission to read your phone data automatically.\n\nTap OK to open Settings, enable "DUIIApp" under Usage Access, then come back and tap Fetch again.',
          [
            {text: 'Cancel', style: 'cancel', onPress: () => setLoading(false)},
            {
              text: 'Open Settings',
              onPress: () => {
                NativeModules.UsageStatsModule.openUsageSettings();
                setLoading(false);
              },
            },
          ],
      ...
      const fetched = {
        usage: data.usage,
        checks: data.checks,
        bedPhone: data.bedPhone,
        sleepHours: data.sleepHours,
      };
      setAutoFetched({...calcDUII(fetched), inputs: fetched, sleepEstimated: data.sleepIsEstimated});
    } catch (e) {
      Alert.alert('Error', 'Could not read device data. Please use Manual Input instead.');
    } finally {
      setLoading(false);
    }
  };
            ...
            <View style={[s.card, {backgroundColor: darkMode ? '#0f2744' : '#eef4ff', borderColor: '#00c49a44'}]}>
              <View style={{flexDirection: 'row', alignItems: 'center', gap: 14}}>
                ...
                <View>
                  ...
                  <Text style={[s.muted, {fontSize: 10, marginTop: 2}]}>Data Science Research, 2026</Text>
                </View>
              </View>
            </View>
